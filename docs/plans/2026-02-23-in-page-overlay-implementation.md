# In-Page Overlay Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Build a Chrome extension that injects a filter modal into Google Photos for bulk photo selection by type, date, and orientation.

**Architecture:** Content script injects a Shadow DOM container into the Google Photos page. The Shadow DOM hosts a modal overlay with filter controls. On submission, the script scrolls through the page and selects matching photos using Ctrl+Click simulation. Service worker stores user preferences.

**Tech Stack:** Chrome Extension (MV3), vanilla JavaScript, Shadow DOM, chrome.storage.local
...
```json
{
  ...
  "version": "0.1.0",
  "description": "Select Google Photos by type, date range, and orientation for bulk cleanup.",
  "permissions": ["storage"],
...
**Step 3: Create minimal background.js**
...
**Step 4: Create minimal content.js**

```javascript
// src/content.js
// Content script for photos.google.com

console.log('Google Photos Cleaner: Content script loaded');
```
...
```css
/* src/styles.css */
/* Styles for Google Photos Cleaner overlay */
/* Will be injected into Shadow DOM */
```

**Step 6: Test extension loads**

1. Open `chrome://extensions`
2. Enable "Developer mode"
3. Click "Load unpacked" → select project root
4. Navigate to `https://photos.google.com`
5. Open DevTools console → verify "Content script loaded" message

**Step 7: Commit**

```bash
git add manifest.json src/
git commit -m "feat: add extension skeleton with manifest and stubs"
```

---

## Task 2: Shadow DOM Container Setup

**Files:**
- Modify: `src/content.js`

**Step 1: Create Shadow DOM host element**

Replace content of `src/content.js`:
...
**Step 2: Test Shadow DOM creation**

1. Reload extension in `chrome://extensions`
2. Refresh Google Photos page
3. Open DevTools → Elements tab
4. Search for `gpc-shadow-host`
5. Verify shadow root exists (shown as `#shadow-root (closed)`)
6. Console should show "Shadow DOM initialized"

**Step 3: Commit**

```bash
git add src/content.js
git commit -m "feat: add Shadow DOM container setup"
```

---

## Task 3: Inject Trigger Button into Header

**Files:**
- Modify: `src/content.js`

**Step 1: Add header detection and button injection**

Add after the Shadow DOM setup code, before the closing `})();`:
...
**Step 2: Test trigger button injection**

1. Reload extension
2. Refresh Google Photos
3. Look for blue "Cleaner" button in header (right side)
4. Click button → console should show "Opening modal..."
5. Click again → console should show "Closing modal..."

**Step 3: Commit**

```bash
git add src/content.js
git commit -m "feat: inject trigger button into Google Photos header"
```

---

## Task 4: Modal Overlay Styles

**Files:**
- Modify: `src/content.js` (update getStyles function)

**Step 1: Replace getStyles() with full modal styles**

Find the `getStyles()` function and replace it with:
...
No visual test yet - styles will be used when modal is rendered in Task 5.

**Step 3: Commit**

```bash
git add src/content.js
git commit -m "feat: add modal overlay styles"
```

---

## Task 5: Modal Overlay HTML and State
...
Add after the `state` object declaration:
...
**Step 2: Replace openModal() and closeModal() with full implementations**
...
```javascript
  ...
  function updateActionButton() {
    ...
    const allTypesSelected = filters.fileType.photos && filters.fileType.videos && filters.fileType.raw;
    const noDateFilter = !filters.dateRange.from && !filters.dateRange.to;
    const noOrientationFilter = filters.orientation === 'any';
    const noTypeFilter = filters.fileType.photos && filters.fileType.videos; // Both main types selected
...
**Step 3: Test modal opens and closes**

1. Reload extension
2. Click "Cleaner" button → modal should appear
3. Click backdrop or X button → modal should close
4. Toggle file type buttons → should highlight/unhighlight
5. Set a date → "Start Selection" button should enable
6. Clear date and leave defaults → button should disable with hint

**Step 4: Commit**

```bash
git add src/content.js
git commit -m "feat: add modal overlay with filter controls"
```

---

## Task 6: DOM Helpers for Photo Detection
...
Add before the `injectTriggerButton` function:
...
**Step 2: Test helpers in console**

1. Reload extension
2. Navigate to Google Photos with some photos visible
3. Open DevTools console and test:

```javascript
// These are exposed via the content script's scope,
// but for testing, check the console logs from the extension
```
...
```bash
git add src/content.js
git commit -m "feat: add DOM helper functions for photo detection"
```
...
**Step 1: Implement startSelection()**

Replace the placeholder `startSelection()` function:
...
**Step 2: Test full selection flow**

1. Reload extension
2. Navigate to Google Photos
3. Click "Cleaner" button
4. Set a date range filter (small range for testing)
5. Click "Start Selection"
6. Watch progress counter increment
7. Verify toast appears when done
8. Verify photos are selected (blue checkmarks visible)

**Step 3: Commit**

```bash
git add src/content.js
git commit -m "feat: implement selection algorithm with progress"
```

---

## Task 8: Service Worker for Preferences

**Files:**
- Modify: `src/background.js`
- Modify: `src/content.js`

**Step 1: Update background.js with storage functions**

Replace `src/background.js` content:
...
**Step 2: Load saved filters on modal open**

In `src/content.js`, update `openModal()` to load preferences:

Add at the start of `openModal()`:
...
```javascript
  async function openModal() {
    ...
    // Rest of existing openModal code...
```

**Step 3: Save filters when starting selection**

In `startSelection()`, add after the validation:
...
```javascript
  async function startSelection() {
    ...
    // Rest of existing startSelection code...
```

**Step 4: Test preference persistence**

1. Reload extension
2. Open modal, set some filters
3. Start a selection (or just close)
4. Reload the Google Photos page
5. Open modal again → filters should be restored

**Step 5: Commit**

```bash
git add src/background.js src/content.js
git commit -m "feat: add preference storage via service worker"
```
...
Update `startSelection()` to check for no matches:

At the end of `startSelection()`, before `closeModal()`:
...
1. Test with very restrictive date range (no photos should match)
2. Verify "No photos matched" toast appears

**Step 4: Commit**

```bash
git add src/content.js
git commit -m "feat: add error handling and no-match feedback"
```
...
This plan covers 10 tasks:

1. Project structure and manifest
2. Shadow DOM container setup
3. Trigger button injection
4. Modal overlay styles
5. Modal HTML and state management
6. DOM helpers for photo detection
7. Selection algorithm
8. Service worker for preferences
9. Error handling
10. Documentation

Each task is broken into small steps with complete code. The extension uses Shadow DOM for style isolation and implements all filters (type, date, orientation) as specified in the design.
